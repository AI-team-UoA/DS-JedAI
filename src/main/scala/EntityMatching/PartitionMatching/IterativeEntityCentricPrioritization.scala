    ...
    def getDE9IM: RDD[IM] ={
        ...
        joinedRDD
            ...
            .flatMap { p =>
                ...
                val frequencies = new Array[Short](sourceSize)
                ...
                target
                    .foreach {e2 =>
                        ...
                        util.Arrays.fill(frequencies, 0.toShort)
                        sIndices.flatMap(_._2)foreach(i => frequencies(i) = (frequencies(i) + 1).toShort)
    ...
    def getWeightedDE9IM: RDD[(Double, IM)] ={
        ...
        joinedRDD
            ...
            .flatMap { p =>
                ...
                val comparisons = partitionPQ.dequeueAll.toArray
                var converge = false
                val imList: ListBuffer[(Double, IM)] = ListBuffer[(Double, IM)]()
                while(!converge){
                    converge = true
                    for ((w, (sIndices, e2)) <- comparisons) {
                        if (sIndices.hasNext) {
                            converge = false
                            val e1 = source(sIndices.next())
                            val im = IM(e1, e2)
