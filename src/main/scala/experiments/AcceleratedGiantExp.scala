...
import interlinkers.AcceleratedGIAnt
...
object AcceleratedGiantExp {
    ...
    def main(args: Array[String]): Unit = {
        ...
        val entityTypeType: EntityTypeENUM = EntityTypeENUM.ENHANCED_SPATIAL_ENTITY
        ...
        val targetRDD: RDD[(Int, Entity)] = partitioner.distributeAndTransform(targetSpatialRDD, SpatialEntityType())

        val matchingStartTime = Calendar.getInstance().getTimeInMillis
        val partitionBorder = partitioner.getAdjustedPartitionsBorders(theta)
        val giant = AcceleratedGIAnt(sourceRDD, targetRDD, theta, partitionBorder, partitioner.hashPartitioner)
